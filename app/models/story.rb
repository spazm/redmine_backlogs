class Story < Issue
    ...
    def self.condition(project, sprint_id, extras=[])
      if sprint_id.nil?
        c = ["
          (
            (project_id = ? )
            or ( #{Project.table_name}.lft < ? OR #{Project.table_name}.rgt > ?)
          )
          and tracker_id in (?)
          and fixed_version_id is NULL
          and is_closed = ?"
          , project.id, project.lft, project.rgt, Story.trackers, false]
      else
        c = ["
          (
            (project_id = ? )
          ...
          , project.id, project.lft, project.rgt, Story.trackers, sprint_id]
      end
    ...
    def self.backlog(project, sprint, options={})
      ...
      Story.find(:all,
            :order => Story::ORDER,
            :conditions => Story.condition(project, sprint.id),
            :joins => :status,
      ...
      find(:all,
           :conditions => [
                "(
                    project_id = ? 
                    OR ( 
                           #{Project.table_name}.lft < ? 
                        OR #{Project.table_name}.rgt > ? 
                    )
                 ) 
                 AND updated_on > ? AND tracker_id in (?)"
           , project.id, project.lft, project.rgt, Time.parse(since), trackers],
